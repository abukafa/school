...
class Calendar
{
     ...
     */
    public function addEvent($start, $end, $summary = false, $mask = false, $classes = false, $color = 'red')
    {
        $event = new stdClass();
        ...
        if (is_array($events)) {
            foreach ($events as $event) {
                if (isset($event['start']) && isset($event['end'])) {
                    ...
                    $color = isset($event['color']) ? $event['color'] : false;
                    $this->addEvent($event['start'], $event['end'], $summary, $mask, $classes, $color);
                }
            }
        }
            ...
            if ($events) {
                foreach ($events as $index => $event) {
                    ...
                    if ($event->start->format('Y-m-d') == $running_day->format('Y-m-d')) {
                        $class .= $event->mask ? ' mask-start ' . $event->color : '';
                        ...
                        $class .= $event->mask ? ' mask ' . $event->color : '';

                        # is the current day the start of the event
                    } elseif ($running_day->format('Y-m-d') == $event->end->format('Y-m-d')) {
                        $class .= $event->mask ? ' mask-end ' . $event->color : '';
                    }
                }
            }
...
foreach ($kalender as $row) {
    $events[] = array(
        'start' => date($row['tgl_awal']),
        'end' => date($row['tgl_ahir']),
        'summary' => $row['acara'],
        'mask' => true,
        'color' => $row['warna']
    );
